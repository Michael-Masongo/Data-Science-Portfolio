...
train_original = pd.read_csv(
    "https://raw.githubusercontent.com/Michael-Masongo/Data-Science-Portfolio/refs/heads/main/Credit_Card_Approval_Project/train.csv"
)

test_original = pd.read_csv(
    "https://raw.githubusercontent.com/Michael-Masongo/Data-Science-Portfolio/refs/heads/main/Credit_Card_Approval_Project/test.csv"
)
...
profile_to_predict = [
    ...
    phone_val,  # Phone
    email_val,  # Email
    "to_be_dropped",  # Job title (which will be dropped in the pipeline)
...
def make_prediction():
    ...
    try:
        ...
        with open(model_path, 'rb') as f:
            model = joblib.load(f)
            logger.info("Successfully loaded the local model")
        
        return model.predict(profile_to_pred_prep)
        
    except FileNotFoundError:
        logger.error(f"Model file not found at {model_path}")
        st.error("Model file not found. Please ensure 'gradient_boosting.sav' exists in your project directory.")
        return None
...
if predict_bt:
    ...
    if final_pred is not None:
        ...
        st.error("Prediction failed. Please check if the model file exists.")
